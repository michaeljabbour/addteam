...
def _init_team_yaml_from_current(repo_owner: str, repo_name: str) -> Path:
    ...
    (or expired). Logins only — no display-name lookups. `--sync` run
    immediately after generation is a no-op by construction.
    """
    path = Path("team.yaml")
    if path.exists():
        raise FileExistsError(f"{path} already exists")
    ...
    for login, info in pending.items():
        ...
        comment = "  # pending invite (expired)" if expired else "  # pending invite"
        buckets[role].append((login, f"  - {login}{comment}"))
