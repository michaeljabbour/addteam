...
- **Org team membership management** — `teams:` entries accept a mapping form (`org/slug: {permission, members, maintainers}`) alongside the existing scalar form. Apply ensures listed users are members/maintainers of the team itself (not just repo collaborators); audit reports missing/extra members and role mismatches. Removing extra team members requires the new `--sync-teams` flag (never implied by plain `--sync`) and is subject to the same circuit breaker as repo-collaborator removal, computed per team. A team whose membership can't be read (403/404) is treated as incomplete for that team only — no removals attempted, clear "requires org admin or team maintainer" messaging. Skipped entirely on personal repos.
